...
const componentOf = R.curry((network, a, b) =>
  R.contains(a, allNodes(network, network[b])),
);
  ...
  R.contains(char, stack)
    ...
    : !R.has(char, network) || network[char].decomposition.length === 0
      ...
      : network[char].decomposition
          ...
          .reduce(R.max, 0) + 1;
const sortByDepth = (depths) =>
  R.sort((a, b) =>
    depths[a] < depths[b] ? -1 : depths[a] > depths[b] ? 1 : 0,
  );
const sortByFrequency = (frequencies) =>
  R.sort((a, b) =>
    !R.has(a, frequencies) && !R.has(b, frequencies)
      ? 0
      : !R.has(a, frequencies)
        ? 1
        : !R.has(b, frequencies)
...
const conflate = R.curry((conflateMap, chars) =>
  chars
...
const htfComponentsRaw = htfCharacters
...
const htfComponentUseRaw = htfCharacters
  .concat(htfComponentsRaw)
  .map((char) => network[char].decomposition)
  .into(R.flatten);
const htfComponentUse = htfComponentsRaw
  .map((char) => [
    char,
...
const charactersAndComponents = htfCharacters
  .concat(htfComponents)
  .into(sortByFrequency(frequencies));
const charactersAndComponentsSorted = charactersAndComponents.reduce(
  (r, char) =>
...
const charactersSorted = charactersAndComponentsSorted.filter(
  foundIn(htfCharacters),
);
const componentsSorted = charactersAndComponentsSorted.filter(
  foundIn(htfComponents),
);

const charactersResult = charactersAndComponentsSorted
  .into(R.indexBy(R.identity))
  .into(
    R.map((c) => ({
