...
const idsAnalysis = getIdsFile(
  ...
  ([_, char, dec, cat]) => [
    char,
    R.contains(cat, wrongAnalysisCats) ? "" : dec,
  ],
);
const reCjkDecomp = /^(.+):.+\((.+)\)$/u;
const redefineNumeric = R.curry((mapping, component) =>
  /\d+/.test(component)
    ? mapping[component].decomposition
        .map(redefineNumeric(mapping))
        .into(R.flatten)
        .into(R.uniq)
    : [component],
);
const cjkDecomp = getFile("data/external/cjk-decomp.txt")
  ...
  .into((all) =>
    all.into(R.filter((o) => !/\d+/.test(o.character))).into(
      R.map((o) => ({
        character: o.character,
        decomposition: o.decomposition
          .map(redefineNumeric(all))
          .into(R.flatten)
          .into(R.uniq),
      })),
    ),
